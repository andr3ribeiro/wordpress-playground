...
test('should create temporary site when importing ZIP while on a saved site with no existing temporary site', async ({
	...
	// The system should create a new temporary site and import into it.
	// Wait for the title to show "Temporary Playground"
	await expect(website.page.getByLabel('Playground title')).toContainText(
		'Temporary Playground',
		{ timeout: 60000 }
	);

	// Now verify the saved site is unchanged.
	// Switch back to the saved site
	await website.openSavedPlaygroundsOverlay();
	...
	await playgroundViewport
		.locator('#wp')
		.evaluate((iframe: HTMLIFrameElement) => {
			iframe.contentWindow!.location.href = '/saved-only-marker.php';
		});
